...
const QUIZ_POOL: QuizItem[] = [
  { q: '¿Qué técnica estás usando si le das a la IA 3 ejemplos de input→output antes de tu pedido real?',
    opts: ['Zero-shot prompting, porque el pedido real viene sin ejemplos específicos de salida esperada', 'Few-shot prompting, porque los 3 ejemplos le muestran al modelo el patrón a replicar', 'Chain-of-Thought, porque le estás mostrando los pasos que debe seguir para responder', 'System prompt, porque defines el comportamiento base del modelo antes de empezar a pedir'],
  ...
  { q: '¿Por qué mostrar el razonamiento paso a paso es valioso aunque el resultado final sea el mismo?',
    opts: ['Porque hace que la respuesta sea más larga y la IA cobra más por respuestas largas', 'Porque permite detectar en qué paso exacto hay un error, si el resultado es incorrecto', 'Porque activa una función premium que mejora automáticamente la calidad de las respuestas del modelo', 'Porque los modelos actuales solo razonan correctamente cuando les piden los pasos explícitamente'],
  ...
  { q: '¿Por qué decimos "alucinación" y no "mentira" cuando la IA inventa datos?',
    opts: ['Porque las alucinaciones se consideran errores suaves y las mentiras se consideran errores mucho más graves', 'Porque las mentiras requieren intención consciente, y los LLMs no tienen intenciones ni voluntad', 'Porque "alucinación" suena mucho más científico y técnico en los artículos de investigación académica', 'Porque los ingenieros inventaron esa palabra para ocultar las fallas técnicas de sus propios modelos'],
  ...
  { q: '"Actúa como experto" vs "Actúa como oncólogo con 15 años en hospitales pediátricos de Colombia". ¿Cuál funciona mejor y por qué?',
    opts: ['El primero porque es más corto y la IA siempre procesa mejor los prompts concisos y directos', 'El segundo porque la especificidad del rol guía al modelo a un registro técnico mucho más preciso', 'Ambos funcionan idéntico porque el modelo ya tiene conocimiento entrenado de todo tipo de expertos', 'El primero porque darle demasiado contexto adicional confunde al modelo y reduce la calidad final'],
...
const COMPARE_PAIRS = [
  { ctx: 'Quieres que la IA te ayude a estudiar para un examen de biología sobre el sistema digestivo.',
    opts: [
      ...
      { label: 'Prompt B', text: 'Hola, tengo un examen de biología mañana y necesito que me expliques el sistema digestivo bien completo, con toda la información importante que creas que me puede servir para sacar buena nota. Explícalo lo mejor que puedas porque este tema me cuesta bastante y es una parte grande de la calificación final.' },
    ], correct: 0,
  ...
  { ctx: 'Necesitas que la IA clasifique 20 correos en urgente / normal / spam usando tu criterio personal.',
    opts: [
      { label: 'Prompt A', text: 'Clasifica con mucho cuidado cada uno de estos 20 correos que te voy a pasar en una de estas tres categorías: urgente, normal o spam. Usa tu mejor criterio profesional para decidir en qué categoría va cada correo y entrégame la clasificación lo más ordenada y completa posible: [lista]' },
  ...
  { ctx: 'Tienes un problema de lógica: 3 amigos dividen una cuenta de restaurante con propina y descuento. Quieres la respuesta correcta.',
    opts: [
      ...
      { label: 'Prompt B', text: 'Salimos a comer con dos amigos y la cuenta del restaurante fue de $180.000. Tenemos un cupón con 10% de descuento y queremos dejar 8% de propina, y al final vamos a dividir todo entre las 3 personas. Dime directamente cuánto le toca pagar a cada uno.' },
    ], correct: 0,
