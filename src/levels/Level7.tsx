...
export default function World2Level1() {
  ...
  // Módulo 10 — ranker
  const [rankerOrder, setRankerOrder] = useState<number[]>(shuffle([0, 1, 2]));
  ...
  useEffect(() => {
    ...
    if (step === 5) { setMatchLeft(null); setMatchedL(new Set()); setMatchedR(new Set()); setMatchFb(null); setWrongFlash(null); setRightOrder(shuffle(matchPairs.map((p) => p.right))); }
    if (step === 7) { setSortOrder(shuffle([0, 1, 2, 3, 4])); setSortOk(false); setSortMarks({}); setSortFb(null); }
    if (step === 8) { setTfAnswers({}); setTfChecked(false); }
    if (step === 10) { setRankerOrder(shuffle([0, 1, 2])); setRankerChecked(false); setRankerSwapA(null); setRankerFb(null); }
