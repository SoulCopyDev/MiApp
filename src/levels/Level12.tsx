...
export default function World2Level6({ navigation: propsNavigation, setAllowBack }: LevelProps) {
  ...
  const renderStep = () => {
    switch (step) {
      ...
      case 1: return (<View style={styles.stepContainer}>{tag('Nivel 12 · 18 módulos')}{tag('🎯 Módulo 1 · Zero-shot')}{title('Zero-shot: pedir sin ejemplos')}{body('Funciona para tareas estándar (traducir, definir, calcular). Falla cuando el criterio de calidad es subjetivo y la IA no conoce tu estilo.')}{btn('Continuar →', nextStep)}</View>);
