...
export default function GameLevel2({ navigation: propsNavigation, setAllowBack }: LevelProps) {
  ...
  // Modo examen: actividades que requieren bloqueo de retroceso
  const isExamMode = step === 3 || step === 5 || step === 8 || step === 9 || step === 10 || step === 12 || step === 13 || step === 14 || step === 15;

  // Notificar al padre si se puede hacer retroceso o no
  useEffect(() => {
  ...
  const handleFinish = () => {
    ...
    completeLevel(2, stars, xp); // Mundo 1, Nivel 2
  ...
  const checkDrag3 = () => {
    ...
    if (placedCount < drag3Items.length) {
      showResult(false, `Faltan habilidades. Coloca todas (${drag3Items.length - placedCount} restantes).`);
      return false;
    }
  ...
  const checkLLMDrag = () => {
    ...
    if (placedCount < llmItems.length) {
      showResult(false, `Faltan tareas. Asigna todas (${llmItems.length - placedCount} restantes).`);
      return false;
    }
  ...
  const checkReflect = () => {
    ...
    if (reflectText.trim().length >= 70) {
      ...
      showResult(false, 'Muy corto. Escribe al menos 70 caracteres.');
    }
  };
